package com.niangaoa.mywife.config
...
object WifeConfig {
    ...
    fun init() {
        ...
        file = File("config/MyWife/config.json")
        if (file.isFile) {
            ...
            file.createNewFile()
            try {
                FileOutputStream(file).use { op ->
                    val writer: Writer = OutputStreamWriter(op, StandardCharsets.UTF_8)
